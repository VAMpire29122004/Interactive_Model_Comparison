...
with col2:
    ...
    if predict_btn and canvas_result.image_data is not None:
        ...
        if model_choice == "Random Forest":
            ...
            # --- Visualizing the Forest's Focus ---
            st.markdown("### What the Forest is looking at:")
            # Grab the 784 importance scores and reshape back to 28x28
            importances = rf_model.feature_importances_.reshape(28, 28)
            
            # Plot it as a heatmap
            fig, ax = plt.subplots()
            # cmap="hot" makes the important pixels glow bright yellow/white!
            ...
            # Extract the specific row of weights for the predicted digit
            # and reshape it back into the 28x28 grid
            coefs = l1_model.coef_[int(prediction)].reshape(28, 28)
            
            fig, ax = plt.subplots()
            # cmap="coolwarm" is perfect here:
            # RED pixels = positive weights (increases confidence it's this digit)
            # BLUE pixels = negative weights (decreases confidence)
            # WHITE pixels = weights pushed to exactly 0 by the L1 penalty
